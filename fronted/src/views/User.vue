<template>
    <div>
        <div class="d-flex align-items-center column-gap-2">
            <p class="lead m-0">
                <i class="bi bi-house-door-fill"></i>Foydalanuvchilar
            </p>
...
const roles = ref([
    ...
    let getAllDepos = async () => {
        try {
            ...
            if(res.data) {
                depos = res.data.map(function(depo) { return { text: depo.name, value: depo.id } })
            }
        } catch (error) {
            console.error(error);
        }
