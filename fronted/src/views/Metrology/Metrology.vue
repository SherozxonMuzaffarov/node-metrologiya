<template>
    <div>
        <div class="d-flex align-items-center column-gap-2">
            <p class="lead m-0"><i class="bi bi-house-door-fill"></i>Sexdagi mavjud shablonlar </p>
      ...
      <!-- Table -->
        <BTableSimple striped="true" hover="true" bordered="true" class="mt-4">
            ...
            <BTr v-for="item in Data" :key="item.id">
                ...
                <BTd>{{ item.name }}</BTd>
                <BTd class="btns"
                ><button @click="getOne(item.id)" class="btn btn-primary m-0" >
                    <i class="bi bi-pen-fill"></i></button
                ></BTd>
                <BTd class="btns"
                ><button class="btn btn-danger m-0" @click="deleteItem(item.id)" >
...
const Data = ref([]);
    ...
    const handleOk = async () => {
        try {
            let res = await axios.post("/category/create", FormData.value);
            if (res) {
    ...
    const handleUpdate = async () => {
        ...
        try {
            let res = await axios.patch("/category/update/" + id, FormData.value);
            if (res) {
                getAll();
    ...
    const getOne = async (id) => {
        try {
            let res = await axios.get("/category/one/" + id);
            if (res.data) {
...
//delete
    const deleteItem = async (id) => {
        ...
        if(confirmDelete) {
            try {
                let res = await axios.delete("/category/delete/" + id);
